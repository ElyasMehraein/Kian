...
function getMessageId(event: Pick<UnsignedEvent, 'tags' | 'content'>): string {
  ...
  if (!fromContent) {
    throw new Error('Missing read receipt message id');
  }

  return fromContent;
}

export async function handleRead(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {
  const messageId = getMessageId(event);

  await receiptsRepo.addRead(messageId);
  await messagesRepo.updateStatus(messageId, 'read');
  messageEvents.emit();
}

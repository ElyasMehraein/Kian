...
function getMessageId(event: Pick<UnsignedEvent, 'tags' | 'content'>): string {
  ...
  if (!fromContent) {
    throw new Error('Missing product request fulfilled message id');
  }

  return fromContent;
}

export async function handleProductRequestFulfilled(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {

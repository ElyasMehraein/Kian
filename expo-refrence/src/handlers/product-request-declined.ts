...
function getMessageId(event: Pick<UnsignedEvent, 'tags' | 'content'>): string {
  ...
  if (!fromContent) {
    throw new Error('Missing product request declined message id');
  }

  return fromContent;
}

export async function handleProductRequestDeclined(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {

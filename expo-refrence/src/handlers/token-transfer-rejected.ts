...
export async function handleTokenTransferRejected(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {
  const messageId = getMessageId(event);

  await messagesRepo.updateTokenTransferStatus(messageId, 'rejected');
  messageEvents.emit();
}

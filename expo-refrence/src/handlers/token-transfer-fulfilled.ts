...
export async function handleTokenTransferFulfilled(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {
  const messageId = getMessageId(event);

  await messagesRepo.updateTokenTransferStatus(messageId, 'fulfilled');
  messageEvents.emit();
}

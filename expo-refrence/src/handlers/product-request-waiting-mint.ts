...
function getMessageId(event: Pick<UnsignedEvent, 'tags' | 'content'>): string {
  ...
  if (!fromContent) {
    throw new Error('Missing product request waiting message id');
  }

  return fromContent;
}

export async function handleProductRequestWaitingMint(event: Pick<UnsignedEvent, 'tags' | 'content'>): Promise<void> {
  const messageId = getMessageId(event);

  await messagesRepo.updateRequestStatus(messageId, 'waiting_mint');
  messageEvents.emit();
}
